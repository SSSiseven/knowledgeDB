...
采用**语义分块**策略，失败时自动回退递归字符分割：

**语义分块流程：**

```
全文 → 句子切分 → 每句384维向量化 → 相邻句余弦相似度
→ 滑动窗口检测断裂点（低于均值75%断开） → 合并成块
→ 最小300字兜底 / 最大3000字截断
```

| 参数 | 值 | 说明 |
|------|------|------|
| SEMANTIC_MIN_CHUNK_CHARS | 300 字符 | 小于此不单独成块，避免碎片化 |
| SEMANTIC_MAX_CHUNK_CHARS | 3000 字符 | 超过此强制截断 |
| SIMILARITY_THRESHOLD_RATIO | 0.75 | 低于滑动均值 75% 视为语义断裂 |
| SMOOTHING_WINDOW | 5 | 相似度平滑窗口，滤除噪声波动 |

**回退方案（递归字符分割）：**

语义分块失败时（如向量化异常），自动回退到规则分块：
...
一篇 30 页论文通常切为 20-60 个自适应块（语义分块）或 60-70 个固定块（回退方案），全部存入 ChromaDB。检索时取与问题最相关的 8-10 个块发给 LLM。
...
用户问题
   │
   ├── 向量检索 (ChromaDB 余弦搜索, 权重70%)
   │   问题 → 384维向量 → top20 最相似分块
   │
   ├── 关键词检索 (SQLite LIKE, 权重30%)
   │   问题关键词 → 标题/摘要匹配 → top20 论文
   │
   └── RRF 融合排序 → top10 结果 → 补全标题/arxiv → 发给 LLM
```

RRF (Reciprocal Rank Fusion) 公式：`score = Σ weight / (60 + rank)`，同一分块被两路同时命中时分数叠加。向量检索擅长语义泛化，关键词检索保证专有名词精确命中，互补融合。
